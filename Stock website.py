...
if page == "Dashboard":
    ...
    with tab1:
        ...
        fig.add_trace(go.Bar(x=df['Date'], y=df['Total_Net_Buy'], name='Net Buy (Chips)', marker_color=chip_colors, yaxis='y2', opacity=0.3))
    ...
    with tab2:
        ...
        for i in range(min(50, mc_sims)):
            fig_mc.add_trace(go.Scatter(y=simulation_paths[:, i], mode='lines', line=dict(width=1, color='rgba(0,100,255,0.2)'), showlegend=False))
        fig_mc.add_trace(go.Scatter(y=np.mean(simulation_paths, axis=1), mode='lines', name='Mean Path', line=dict(color='orange', width=3)))
...
# --- PAGE 2: CORRELATION LAB ---
    ...
    st.markdown("Compare **TW Stocks** vs **US Stocks**. *Tickers with digits only -> Auto-add .TW suffix.*")
    ...
    if run_btn:
        ...
        try:
            ...
            with c2:
                ...
                fig_corr = px.imshow(corr_matrix, text_auto=".2f", aspect="equal", color_continuous_scale="RdBu_r", zmin=-1, zmax=1)
            ...
            fig_norm.update_layout(hovermode="x unified", height=600, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
...
# --- PAGE 3: ETF CONSENSUS HEATMAP ---
    ...
    if not csv_files:
        ...
        if df_list:
            ...
            pivot_df = all_holdings.pivot_table(index='代號', columns='ETF_Code', values='權重', aggfunc='sum').fillna(0)
            ...
            fig_heat = px.imshow(
                heatmap_data, labels=dict(x="主動型 ETF", y="標的", color="權重(%)"),
...
# --- PAGE 4: ALPHA TERMINAL ---
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["🔥 橫截面共識熱力圖", "🎯 完美擊球點掃描", "🌊 3D 流動性衝擊矩陣", "🕵️ 個股深度透視 (Tearsheet)"])
    ...
    with tab1:
        ...
        if not df_consensus.empty:
            ...
            fig = px.treemap(
                plot_df, path=[px.Constant("全市場高共識標的"), '持有家數', '標籤'], 
                values='總權重', color='持有家數', color_continuous_scale='Turbo',
    ...
    with tab2:
        ...
        if not df_confluence.empty:
            st.dataframe(df_confluence.rename(columns={'T1_Count': '昨日持有家數', 'T0_Count': '今日持有家數'}), use_container_width=True, hide_index=True)
    ...
    with tab4:
        st.subheader(f"🕵️ {ticker_input} 機構微結構體檢表 (Tearsheet)")
        
        cons_data = df_consensus[df_consensus['代號'] == ticker_input]
        imp_data = df_impact[df_impact['代號'] == ticker_input]
        conf_data = df_confluence[df_confluence['代號'] == ticker_input]

        if cons_data.empty and imp_data.empty and conf_data.empty:
            st.warning(f"目前 {ticker_input} 不在 7 大主動 ETF 的觀測雷達內，屬於純散戶或外資控盤標的。")
        else:
            col1, col2, col3 = st.columns(3)
            with col1:
                st.markdown("### 📊 V5 機構共識")
                if not cons_data.empty:
                    st.metric("持有家數", f"{cons_data['持有家數'].values[0]} 家")
                    st.metric("總權重", f"{cons_data['總權重'].values[0]:.2f}%")
                    st.write("**持有清單:**")
                    st.caption(cons_data['持有ETF名單'].values[0] if '持有ETF名單' in cons_data.columns else "無明細資料")
                else:
                    st.write("❌ 尚未進入高共識區")

            with col2:
                st.markdown("### 🌊 V7 流動性衝擊")
                if not imp_data.empty:
                    status = imp_data['狀態'].values[0]
                    if "🌱" in status or "🛡️" in status: st.success(status)
                    elif "🚨" in status or "💀" in status: st.error(status)
                    else: st.info(status)
                        
                    st.metric("投信參與率", f"{imp_data['投信參與率(%)'].values[0]:.2f}%")
                    st.metric("外資參與率", f"{imp_data['外資參與率(%)'].values[0]:.2f}%")
                    st.write(f"**投信淨買超:** {imp_data['投信淨買超(張)'].values[0]:,} 張")
                    st.write(f"**外資淨買超:** {imp_data['外資淨買超(張)'].values[0]:,} 張")
                else:
                    st.write("⏸️ 今日無顯著資金衝擊")

            with col3:
                st.markdown("### 🎯 V9 點火共振")
                if not conf_data.empty:
                    st.metric("今日新增點火主力", f"{conf_data['點火主力'].values[0]}")
                    st.write("**共識擴散度 (昨日 ➡️ 今日)**")
                    st.write(f"### {conf_data['T1_Count'].values[0]} 家 ➡️ {conf_data['T0_Count'].values[0]} 家")
                else:
                    st.write("⏸️ 今日無新主力點火跡象")